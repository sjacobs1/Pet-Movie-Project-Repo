...
struct Constants {
    struct URLs {
        ...
        static let searchMoviesBaseURL = "https://api.themoviedb.org/3/search/movie"

        static func movieDetailsURL(movieID: Int) -> String {
            return "\(baseURL)\(movieID)?api_key=\(apiKey)"
        }

        static func searchMoviesURL(query: String) -> String {
            let encodedQuery = query.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed) ?? ""
            return "\(searchMoviesBaseURL)?query=\(encodedQuery)&include_adult=false&api_key=\(apiKey)"
        }
    }
